...
HudPipeline::HudPipeline(VkDevice device, VkFormat colorFormat, VkFormat depthFormat)
    : device_(device) {
    const auto shaderDir = executableDir() / "shaders";
    VkShaderModule vert = loadShaderModule(device_, shaderDir / "hud.vert.spv");
    VkShaderModule frag = loadShaderModule(device_, shaderDir / "hud.frag.spv");

    VkPipelineLayoutCreateInfo layoutInfo{};
    layoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    if (vkCreatePipelineLayout(device_, &layoutInfo, nullptr, &layout_) != VK_SUCCESS) {
        throw std::runtime_error("vkCreatePipelineLayout 失败 (hud)");
    }
    ...
    auto vBinding = HudVertex::binding();
    auto vAttribs = HudVertex::attributes();
    ...
    if (res != VK_SUCCESS) {
        throw std::runtime_error("vkCreateGraphicsPipelines 失败 (hud)");
    }
    spdlog::info("HUD pipeline 已创建");
}

HudPipeline::~HudPipeline() {
    if (pipeline_ != VK_NULL_HANDLE) vkDestroyPipeline(device_, pipeline_, nullptr);
    if (layout_ != VK_NULL_HANDLE) vkDestroyPipelineLayout(device_, layout_, nullptr);
    spdlog::info("HUD pipeline 已销毁");
}
