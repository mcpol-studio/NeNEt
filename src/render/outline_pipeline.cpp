...
OutlinePipeline::OutlinePipeline(VkDevice device, VkFormat colorFormat, VkFormat depthFormat)
    : device_(device) {
    const auto shaderDir = executableDir() / "shaders";
    VkShaderModule vert = loadShaderModule(device_, shaderDir / "outline.vert.spv");
    VkShaderModule frag = loadShaderModule(device_, shaderDir / "outline.frag.spv");
    ...
    if (vkCreatePipelineLayout(device_, &layoutInfo, nullptr, &layout_) != VK_SUCCESS) {
        throw std::runtime_error("vkCreatePipelineLayout 失败 (outline)");
    }
    ...
    if (res != VK_SUCCESS) {
        throw std::runtime_error("vkCreateGraphicsPipelines 失败 (outline)");
    }
    spdlog::info("Outline pipeline 已创建");
}

OutlinePipeline::~OutlinePipeline() {
    if (pipeline_ != VK_NULL_HANDLE) vkDestroyPipeline(device_, pipeline_, nullptr);
    if (layout_ != VK_NULL_HANDLE) vkDestroyPipelineLayout(device_, layout_, nullptr);
    spdlog::info("Outline pipeline 已销毁");
}
