...
WallpaperPipeline::WallpaperPipeline(VkDevice device, VkFormat colorFormat, VkFormat depthFormat)
    : device_(device) {
    const auto shaderDir = executableDir() / "shaders";
    VkShaderModule vert = loadShaderModule(device_, shaderDir / "wallpaper.vert.spv");
    VkShaderModule frag = loadShaderModule(device_, shaderDir / "wallpaper.frag.spv");
    ...
    if (vkCreateDescriptorSetLayout(device_, &setInfo, nullptr, &setLayout_) != VK_SUCCESS) {
        throw std::runtime_error("vkCreateDescriptorSetLayout 失败 (wallpaper)");
    }

    VkPushConstantRange pcRange{};
    pcRange.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
    pcRange.offset = 0;
    pcRange.size = sizeof(WallpaperPush);
    ...
    if (vkCreatePipelineLayout(device_, &layoutInfo, nullptr, &layout_) != VK_SUCCESS) {
        throw std::runtime_error("vkCreatePipelineLayout 失败 (wallpaper)");
    }
    ...
    if (res != VK_SUCCESS) {
        throw std::runtime_error("vkCreateGraphicsPipelines 失败 (wallpaper)");
    }
    spdlog::info("Wallpaper pipeline 已创建");
}
...
WallpaperPipeline::~WallpaperPipeline() {
    ...
    spdlog::info("Wallpaper pipeline 已销毁");
}
