...
HotbarPipeline::HotbarPipeline(VkDevice device, VkFormat colorFormat, VkFormat depthFormat) : device_(device) {
    const auto shaderDir = executableDir() / "shaders";
    VkShaderModule vert = loadShaderModule(device_, shaderDir / "hotbar.vert.spv");
    VkShaderModule frag = loadShaderModule(device_, shaderDir / "hotbar.frag.spv");

    VkPushConstantRange pcRange{};
    pcRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
    pcRange.offset = 0;
    pcRange.size = sizeof(HotbarPush);
    ...
    if (vkCreatePipelineLayout(device_, &layoutInfo, nullptr, &layout_) != VK_SUCCESS) {
        throw std::runtime_error("vkCreatePipelineLayout 失败 (hotbar)");
    }
    ...
    if (res != VK_SUCCESS) {
        throw std::runtime_error("vkCreateGraphicsPipelines 失败 (hotbar) code=" + std::to_string(res));
    }
    spdlog::info("Hotbar pipeline 已创建");
}

HotbarPipeline::~HotbarPipeline() {
    if (pipeline_ != VK_NULL_HANDLE) vkDestroyPipeline(device_, pipeline_, nullptr);
    if (layout_ != VK_NULL_HANDLE) vkDestroyPipelineLayout(device_, layout_, nullptr);
    spdlog::info("Hotbar pipeline 已销毁");
}
